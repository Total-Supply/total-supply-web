...
import { ArrowRight, Image as ImageIcon, Info, Trash2, X } from 'lucide-react'
  ...
  return (
    <Dialog.Root
      ...
      onOpenChange={(details) =>
        !details.open && !isSubmitting && handleClose()
      }
    >
        ...
        <Dialog.Content className="bg-card border border-border rounded-xl shadow-lg animate-in fade-in-0 zoom-in-95 duration-200 max-w-2xl">
          ...
          <Dialog.Header className="border-b border-border pb-4">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500/20 to-purple-600/10 ring-1 ring-purple-500/30">
          ...
          <Dialog.Body className="space-y-4 py-6">
            <div className="rounded-lg bg-blue-500/10 p-3 border border-blue-500/20">
              <div className="flex gap-2.5">
                <Info className="h-4 w-4 text-blue-500 mt-0.5 flex-shrink-0" />
                <p className="text-sm text-blue-700 dark:text-blue-300">
                  Keep the customer informed by sharing progress updates with
                  photos and notes. This builds trust and satisfaction.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="progress-notes"
                placeholder="E.g., 'Finished cleaning the living room and kitchen. Moving to bedrooms next...'"
                value={notes}
              ...
              <p className="text-xs text-muted-foreground">
                Describe what you&apos;ve completed so far
              </p>
            </div>
                ...
                <div className="grid grid-cols-3 gap-3 mb-3">
                  {photos.map((url, index) => (
                    <div key={index} className="relative group">
                      ...
                      <div className="absolute bottom-1.5 left-1.5 rounded bg-black/60 px-2 py-0.5 text-xs font-medium text-white">
                        {index + 1}
                      </div>
                    </div>
                  ))}
