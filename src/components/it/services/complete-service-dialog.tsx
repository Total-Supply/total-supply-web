...
export function CompleteServiceDialog({
  ...
  const handleSubmit = () => {
    ...
    if (photos.length === 0) {
      setError('Please upload at least 1 after photo')
      return
    }
    if (!completionNotes.trim() || completionNotes.trim().length < 10) {
      setError('Completion notes are required (minimum 10 characters)')
      return
    }
    if (!solutionSummary.trim() || solutionSummary.trim().length < 10) {
      setError('Solution summary is required (minimum 10 characters)')
      return
    }
    ...
    onSubmit({
      ...
      followUpRecommendations: followUps.trim() || undefined,
      photos,
    })
    setCompletionNotes('')
    setSolutionSummary('')
  ...
  return (
    ...
    >
      ...
      <Dialog.Positioner>
        <Dialog.Content className="bg-card border border-border rounded-xl shadow-lg animate-in fade-in-0 zoom-in-95 duration-200 max-w-2xl max-h-[90vh] overflow-y-auto">
          <Dialog.CloseTrigger
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">
                  Finalize the service resolution
                </p>
              </div>
            </div>
          </Dialog.Header>
          ...
          <Dialog.Body className="py-6">
            <div className="space-y-4">
                ...
                <div className="flex gap-2.5">
                  ...
                  <div className="text-sm text-amber-700 dark:text-amber-300">
                    ...
                    <ul className="space-y-0.5 text-xs">
                      <li>• At least 1 after photo showing resolved issue</li>
                      <li>
                        • Detailed completion notes (minimum 10 characters)
                      </li>
                      <li>• Solution summary for documentation</li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="completion-notes"
                  placeholder="E.g., 'Updated network drivers, reconfigured WiFi settings, tested connectivity. All systems operational. Customer verified internet access working.'"
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  Solution Summary <span className="text-red-500">*</span>
                </label>
                <Textarea
                  id="solution-summary"
                  placeholder="E.g., 'Network connectivity restored by updating WiFi adapter drivers and reconfiguring router settings.'"
                  value={solutionSummary}
                  onChange={(e) => {
                    setSolutionSummary(e.target.value)
                    setError('')
                  }}
                  rows={3}
                  required
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {solutionSummary.length} characters (minimum 10 required)
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="follow-ups"
                  placeholder="E.g., 'Schedule quarterly system maintenance. Consider upgrading to WiFi 6 router for better performance.'"
                  value={followUps}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Suggest preventive measures or future improvements
                </p>
              </div>
              ...
              <div className="space-y-2">
                    ...
                    {photos.map((url, index) => (
                      <div key={index} className="relative group">
                        <img
                          src={url}
                          alt={`After ${index + 1}`}
                          className="h-20 w-full rounded-lg object-cover ring-2 ring-emerald-500/30"
                        />
                        <button
                        ...
                        <div className="absolute bottom-1 left-1 rounded-full bg-emerald-500 px-1.5 py-0.5 text-xs font-semibold text-white">
                          {index + 1}
                        </div>
                      </div>
                    ))}
                ...
                <p className="text-xs text-muted-foreground">
                  {photos.length}/5 photos uploaded (minimum 1 required)
                </p>
              </div>
            </div>
          </Dialog.Body>
