...
async function handler(
  ...
  const service = await prisma.serviceRequest.findUnique({
    ...
    select: {
      ...
      assignments: {
        ...
        take: 1,
        select: { id: true, staffId: true, acceptedAt: true },
      },
    },
  })
  ...
  if (service.status !== 'ASSIGNED') {
    throw new ValidationError('Service must be assigned to start')
  }

  if (!assignment.acceptedAt) {
    throw new ValidationError('Accept the assignment before starting')
  }
  ...
  if (service.customer.email) {
    ...
    try {
      ...
      console.error('Service start email failed', error)
    }
  }

  return ApiResponse.success(updated, 'Service started')
}
