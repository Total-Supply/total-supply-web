...
import { buildServiceAcceptedEmail, sendEmail } from '@/src/lib/email'
import prisma from '@/src/lib/prisma'
import { requireRole } from '@/src/middleware/auth'
import { withErrorHandler } from '@/src/middleware/error-handler'
import { NextRequest } from 'next/server'

const MAX_ACTIVE_ASSIGNMENTS = Number(process.env.CLEANER_MAX_ACTIVE || 3)
...
async function handler(
  ...
  if (service.customer.email) {
    ...
    const { html, text } = buildServiceAcceptedEmail({
    ...
    try {
      await sendWithRetry(3, () =>
        sendEmail({
          ...
          subject: `Cleaner accepted your request`,
          html,
          text,
        }),
      )
    } catch (error) {
      console.error('Service accepted email failed', error)
    }
  }

  return ApiResponse.success(updated, 'Service accepted')
}
