...
import { itCompleteSchema } from '@/src/lib/validations/service.schema'
...
async function handler(
  ...
  const data = await validateBody(body, itCompleteSchema)
    ...
    await tx.auditLog.create({
      data: {
        ...
        details: {
          ...
          completionNotes: data.completionNotes,
          solutionSummary: data.solutionSummary,
          followUpRecommendations: data.followUpRecommendations ?? null,
  ...
  if (service.customer.email) {
    ...
    const { html, text } = buildServiceCompletedEmail({
      ...
      notes: `${data.completionNotes}\n\nSolution: ${data.solutionSummary}\nFollow-ups: ${data.followUpRecommendations || 'None'}`,
    ...
    try {
      await sendWithRetry(3, () =>
        sendEmail({
          ...
          subject: `IT service completed`,
          html,
          text,
        }),
      )
    } catch (error) {
      console.error('IT completion email failed', error)
    }
  }

  return ApiResponse.success(updated, 'IT service completed')
}
