...
/**
 * Complete Service Request (Cleaning) - Admin
 *
 * @description Marks the service request as RESOLVED and saves AFTER photos.
 *
 * @pathParams ServiceIdParams
 * @body CompleteServiceBody
 * @response 200:ServiceActionSuccessResponse:Service completed
...
export const POST = withErrorHandler(async function POST(
  ...
  const data = await validateBody(body, CompleteServiceBody)
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.serviceRequest.update({
      where: { id: serviceId },
      data: { status: 'RESOLVED', notes: data.notes },
    })
