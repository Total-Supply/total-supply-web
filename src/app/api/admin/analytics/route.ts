...
import { AnalyticsSuccessResponse } from '@/src/lib/schemas/analytics'
 ...
 *
 * @description Retrieves comprehensive analytics including user counts, order statistics,
 * service metrics, revenue trends (last 30 days), and recent activity. Supports optional
 * date filtering via query parameters. Only accessible to users with ADMIN role.
 *
 * @params AnalyticsQuery
 * @response 200:AnalyticsSuccessResponse:Successful response with analytics data
 * @responseSet auth
 *
 * @auth bearer
 * @tag Admin
...
async function handler(request: NextRequest) {
  ...
  const validated = AnalyticsSuccessResponse.parse(responseData)
